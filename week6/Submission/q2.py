    ...
    for i in range(N):
        for j in range(N):
            ...
            for k in range(N):
                for l in range(N):
                    next_pos = (l + 1) % N
                    s += -A * (np.sum(state[i, :]) - 1)
                    s += -B * (np.sum(state[:, j]) - 1)
                    s += -C * distances[i, k] * state[k, next_pos]
            new_state[i, j] = 1 if s < 0 else 0
    return new_state
...
def validate_tour(state):
    return np.all(np.sum(state, axis=1) == 1) and np.all(np.sum(state, axis=0) == 1)
